...
import { getTsCandidates, updateTsCandidate } from '../../../redux/actions/tsCandidateActions'
import { getTsJobs } from '../../../redux/actions/tsJobActions'
import CandidateDetails from './CandidateDetails'
import Notifier from '../../aside/Notifier'

class TsPanel extends Component {
  constructor(props){
    super(props);
    this.state = {
  ...
  tableData = stat =>
    <Table className="interview-table">
      ...
      {this.state.data.map(
        item => {
          if((item.status === stat) && (item.panelName === this.member)){
            return <tr
      ...
      <div className={shrink}>
        ...
        ?(<Container className="card container-card">
          <input
            ...
            placeholder="Search" />        
          <h5 className="bold pointer center pt-2">Panel</h5>
          <Row>
...
export default connect(
  mapStateToProps,
  { getTsCandidates, updateTsCandidate, getTsJobs }
)(withRouter(TsPanel));