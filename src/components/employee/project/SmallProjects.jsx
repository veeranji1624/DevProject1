...
import SmallProjectDetails from './SmallProjectDetails'
...
class SmallProjects extends Component {
  ...
  onSubmit(e){
    ...
    //this.props.updateProject(data[0])
  ...
  render() {
    ...
    return (
      <div className={shrink}>
        {popup && <SmallProjectDetails detail={selection} update={this.beforeUpdate} back={this.back} />}
        ...
        ?(<Container className="card container-card">
          ...
          <h5 className="bold pointer center pt-2">Small Projects</h5>
          <Row>
            <Col
              md="4"
            ...
            >
              ...
              {
                 data && data.map(item => {
                  if((item.projectType === 'Small') && (item.status === 'New')){
                  return <ul
                    key={item.projCode}
            ...
            >
              ...
              {
                 data && data.map(item => {
                  if((item.projectType === 'Small') && (item.status === 'WIP')){
                  return <ul
                    key={item.projCode}
                    onClick={() => this.select(item)}
                    draggable
            ...
            >
              ...
              {
                 data && data.map(item => {
                  if((item.projectType === 'Small') && (item.status !== 'New') && (item.status !== 'WIP')){
                  return <ul
                    key={item.projCode}
                    onClick={() => this.select(item)}
                    draggable
